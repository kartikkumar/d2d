...
#ifndef D2D_LAMBERT_TRANSFER_HPP
#define D2D_LAMBERT_TRANSFER_HPP
...
//! Execute lambert_transfer.
/*!
 * Executes a single Lambert transfer to compute \f$\Delta V\f$ for departure and arrival with
 * respect to given TLE catalog objects. The transfer is modelled as conic sections. The Lambert
...
//! Input for lambert_transfer application mode.
/*!
 * Data struct containing all valid input parameters to execute the lambert_transfer application
 * mode. This struct is populated by the checkLambertTransferInput() function and can be used to
