...
//! Execute sgp4_propagator.
void executeSGP4Propagator( const rapidjson::Document& config )
{
    // Verify config parameters. Exception is thrown if any of the parameters are missing.
    const SGP4PropagatorInput input = checkSGP4PropagatorInput( config );
    ...
    // Create sgp4_propagator_results table in SQLite database.
    std::cout << "Creating SQLite database table if needed ... " << std::endl;
    createSGP4PropagatorTable( database );
    ...
    // Setup insert query to insert data into sgp4_propagator_results table.
    std::ostringstream sgp4PropagatorTableInsert;
    sgp4PropagatorTableInsert << "INSERT INTO sgp4_propagator_results VALUES ("
        << "NULL,"
        << ":lambert_transfer_id,"
...
//! Check sgp4_propagator input parameters.
SGP4PropagatorInput checkSGP4PropagatorInput( const rapidjson::Document& config )
{
    ...
    return SGP4PropagatorInput( catalogPath,
                                databasePath,
                                shortlistLength,
                                shortlistPath );
}

//! Create sgp4_propagator table.
void createSGP4PropagatorTable( SQLite::Database& database )
{
    ...
    database.exec( "DROP TABLE IF EXISTS sgp4_propagator_results;" );

    // Set up SQL command to create table to store SGP4 propagator results.
    std::ostringstream sgp4PropagatorTableCreate;
    sgp4PropagatorTableCreate
        << "CREATE TABLE sgp4_propagator_results ("
    ...
    database.exec( sgp4PropagatorTableCreate.str( ).c_str( ) );

    // Execute command to create index on transfer arrival position error column.
    ...
    arrivalPositionErrorIndexCreate << "CREATE INDEX IF NOT EXISTS \"arrival_position_error\" on "
                                    << "sgp4_propagator_results (arrival_position_error ASC);";
    database.exec( arrivalPositionErrorIndexCreate.str( ).c_str( ) );

    if ( !database.tableExists( "sgp4_propagator_results" ) )
    {
        throw std::runtime_error( "ERROR: Creating table 'sgp4_propagator_results' failed!" );
    }
}
